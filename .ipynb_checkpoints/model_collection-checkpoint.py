...
class RidgeRegression:
    ...
    def fit(self, X, y):
        ...
        I = np.eye(n_features)
        I[0, 0] = 0
        theta_best = np.linalg.inv(X_b.T @ X_b + self.alpha * I) @ X_b.T @ y
        self.intercept_ = theta_best[0]
        self.coef_ = theta_best[1:]
        return self
    ...
    def __repr__(self):
        return f"RidgeRegression(alpha={self.alpha})"
...
class XGBoostRegressor:
    def __init__(self, params, random_seed=None):
        self.params = defaultdict(none_default, params)
    ...
    def fit(self, X, y, objective, num_boost_round, verbose=False):
        ...
        for i in range(num_boost_round):
            ...
            booster = TreeBooster(X, gradients, hessians, self.params, self.max_depth, sample_idxs, feature_importance=self.feature_importance_)
...
class TreeBooster:
    def __init__(self, X, g, h, params, max_depth, idxs=None, feature_importance=None):
        self.params = params
        self.max_depth = max_depth
        self.feature_importance = feature_importance if feature_importance is not None else defaultdict(float)
    ...
    def _maybe_insert_child_nodes(self):
        ...
        self.left = TreeBooster(self.X, self.g, self.h, self.params, self.max_depth - 1, self.idxs[left_idx], self.feature_importance)
        self.right = TreeBooster(self.X, self.g, self.h, self.params, self.max_depth - 1, self.idxs[right_idx], self.feature_importance)
    ...
    def _find_better_split(self, feature_idx):
        ...
        if self.best_score_so_far > 0:
            self.feature_importance[self.split_feature_idx] += self.best_score_so_far
